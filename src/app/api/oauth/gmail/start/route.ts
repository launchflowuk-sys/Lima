...
import { isGmailConfigured, buildConsentUrl, signOAuthState } from "@/server/email/providers/gmail-oauth";
import { env } from "@/env";

/**
 * Start the Gmail OAuth connect flow: authorize the user for the target business, then redirect to
 * Google's consent screen with a signed `state`. `businessId` comes from the query string.
 */
export async function GET(req: Request): Promise<Response> {
  ...
  if (!isGmailConfigured()) {
    return NextResponse.redirect(new URL("/mailboxes?error=gmail_not_configured", env.APP_URL));
  }
