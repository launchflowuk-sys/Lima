...
/**
 * Persist a Gmail connection after a successful OAuth exchange. Upserts by (businessId, emailAddress)
 * so re-connecting the same inbox refreshes tokens instead of erroring on the unique constraint. All
 * token material is encrypted before it touches the DB. Kept separate from the IMAP-heavy
 * mailboxes/service.ts to keep both files small and focused.
 */
...
export async function connectGmailMailbox(input: ConnectGmailInput): Promise<{ mailboxId: string }> {
  ...
  if (existing[0]) {
    ...
    await db
      ...
      .set({
        ...
        // Only overwrite the refresh token when Google returned a fresh one (it often omits it on re-consent).
  ...
  await db.insert(mailboxHealthEvents).values({ mailboxId, kind: "connected", detail: { provider: "gmail" } });
  await recordAudit({
    businessId,
    actorUserId: user.id,
    ...
    entityType: "mailbox",
    entityId: mailboxId,
    metadata: { provider: "gmail", emailAddress },
  });
