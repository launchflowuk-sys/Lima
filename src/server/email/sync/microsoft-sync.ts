...
/**
 * Microsoft Graph incremental sync. Reads the stored deltaLink cursor, asks the provider for changed
 * message ids, fetches + stores each one mirroring the Gmail/IMAP path's persistence (dedup by
 * (mailbox, providerMessageId); find-or-create thread by (mailbox, providerThreadId); participants;
 * contact memory; attachment metadata). Advances the cursor to the provider's returned deltaLink.
 * Standalone from imap-sync/gmail-sync by design — never import or mutate those files.
 */

// Cap a single run so a first sync can't pull an unbounded number of messages.
const INITIAL_WINDOW = 50;
// Attachment extensions we refuse to store/process (spec §24) — same list as the IMAP/Gmail paths.
const BLOCKED_EXT = /\.(exe|msi|bat|cmd|com|scr|ps1|js|jar|vbs|dll)$/i;

export async function syncMicrosoftMailbox(mailbox: Mailbox): Promise<{ ingested: number; inboundThreadIds: string[] }> {
  const provider = new MicrosoftProvider(mailbox);
  let ingested = 0;
  ...
  try {
    ...
    for (const messageId of changedMessageIds.slice(0, INITIAL_WINDOW)) {
      try {
        ...
        logger.warn({ err, messageId, mailboxId: mailbox.id }, "Failed to fetch/store Microsoft message");
      }
    }
...
async function storeMessage(
  ...
  // Find or create the thread by the Graph conversation id.
  ...
  if (direction === "inbound" && from?.address) {
    try {
      ...
      logger.warn({ err, mailboxId: mailbox.id }, "Failed to upsert contact from inbound Microsoft message");
    }
  }
...
/** Advance/annotate the sync cursor. A null cursor leaves the stored value untouched (mirrors Gmail/IMAP). */
